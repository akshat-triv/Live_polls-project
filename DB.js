...
exports.save = (obj, which) => {
  let poll = { ...obj };
  connection.query(`INSERT INTO ${which} SET ?`, poll, function (
    err,
    result,
    field
  ) {
...
exports.find = (id, which, callback) => {
  let data = [];
  connection.query(`SELECT * FROM ${which} WHERE ?`, id, function (
    err,
    res,
    field
  ) {
...
exports.deleteAll = (obj, which) => {
  const query = connection.query(`DELETE FROM ${which} WHERE ?`, obj, function (
    err,
    res,
    field
  ) {
...
exports.vote = (id, which) => {
  ...
  connection.query(`UPDATE ${which} SET votes=votes+1 WHERE ?`, tmp, function (
    err,
    res,
    field
  ) {
